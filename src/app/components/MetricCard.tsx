    ...
    >
      ...
      {trend && trendValue && (
        <div className="flex items-center gap-2 text-sm">
          <span className={`${trend === 'up' ? 'text-emerald-500 dark:text-emerald-400' : trend === 'down' ? 'text-red-500 dark:text-red-400' : 'text-gray-400 dark:text-zinc-400'}`}>
            {trend === 'up' ? '↑' : trend === 'down' ? '↓' : '→'} {trendValue}
          </span>
          <span className="text-gray-400 dark:text-zinc-500">vs last session</span>
        </div>
      )}
    </motion.div>
  );
